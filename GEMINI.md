...
- **태그 메시지 작성**: 태그를 푸시할 때는 마지막 태그 이후 작업된 커밋 내역을 정리하고, 그 요약을 태그 메시지로 작성합니다.
- **릴리즈 노트 형식**: 태그 푸시로 릴리즈가 생성될 때는 릴리즈 노트를 한국어로 작성하고, `🐛 버그 수정`, `✨ 기능 추가`, `🎨 디자인 수정`, `📝 문서 수정`, `♻️ 리팩토링`, `🔧 기타 변경` 등의 섹션으로 변경사항을 구분합니다. 변경사항이 없는 섹션은 생략합니다.
- 각 릴리즈 노트 섹션 아래에는 실제 변경사항을 하위 항목으로 작성합니다.
